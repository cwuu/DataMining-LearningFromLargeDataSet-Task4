...
## 			
## 			#TODO rescaling reward (0 -> -0.5 or similar)
## 			#TODO timer of model until it is reset (e.g. after 50 updates it starts from new)
## 			
###################################################################################################
...
alpha = 0.1  #TODO set lower alpha if we reset every day
...
current_time = 1  # place holders, as soon as time changes, we reset all weights
old_time = 1
...
def update(reward):
	...
	"""
	global M, Minv, b, B, current_user_features, current_article, A0, A0inv, b0, beta, theta, zA0z, zA0BA, ABA0BA, current_time, old_time, article_features

	if current_time != old_time:  # start from fresh on a new day
		for article_id, features in article_features.iteritems():
			M[article_id] = np.identity(num_user_features)
			Minv[article_id] = np.identity(num_user_features)
			B[article_id] = np.zeros((num_art_features,num_user_features))
			b[article_id] = np.zeros(num_user_features) #w
...
def recommend(time, user_features, choices):
	...
	"""
	global M, Minv, b, B, alpha, current_user_features, current_article, zA0z, zA0BA, ABA0BA, theta, current_time
